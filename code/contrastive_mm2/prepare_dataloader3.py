...
def save_all(save_path, all_mean_embeddings, filenames, in_train_set, all_sentences, all_full_embeddings, segment_tsps):
    print("[del] in save_all")

    # Sort everything according to sentence level
    lengths = np.array([len(s) for s in all_sentences])
    print("Lengths begin : ", lengths[:128])
    print("Lengths end   : ", lengths[-128:])
    print("segs: " ,segment_tsps[:10])
    ...
    print("Lengths begin : ",  np.array([len(s) for s in all_sentences][:128]))
    print("Lengths end   : ",  np.array([len(s) for s in all_sentences][-128:]))
    print("segs: " ,segment_tsps[:10])
    ...
    # f.create_dataset("mean_embeddings", data=all_mean_embeddings)
    ...
    for embed_idx, full_embeds in enumerate(all_full_embeddings):
        ...
        # f.create_dataset(str(embed_idx), data=full_embeds)
...
if __name__ == "__main__":
    ...
    save_every = 9
    save_count = 0
    for count, idx in enumerate(process_idxs):
        ...
        if count % 10 == 0:
            print("\t processing {}/{}".format(count, len(outer_folders)))
