...
class simple_gru_v2(nn.Module):
    """
    :param tokenizer_name_or_path: Name or path of the tokenizer. When None, then model_name_or_path is used
    """
    def __init__(self,  input_dim, hidden_dim, num_layers, output_dim=768, dropout_prob=0.1, 
                use_softmax = False, pad_pack = True, device=None):
        super(simple_gru_v2, self).__init__()
        ...
        self.use_relu = True        # TODO: test usabililty
    ...
    def init_hidden(self, batch_size):
        ...
        hidden = weight.new(self.num_layers, batch_size, self.hidden_dim).zero_().to(self._target_device)
        return hidden
    ...
    def forward(self, audio_seq):
        ...
        out, h = self.gru(features, h)
        
        if self.pad_pack:
            # undo the packing operation
            out, test = pad_packed_sequence(out, batch_first=True)
...
class LSTMNet(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, n_layers, device=None, drop_prob=0.2):
    ...
    def init_hidden(self, batch_size):
        weight = next(self.parameters()).data
        hidden = (weight.new(self.n_layers, batch_size, self.hidden_dim).zero_().to(self._target_device),
                  weight.new(self.n_layers, batch_size, self.hidden_dim).zero_().to(self._target_device))
        return hidden
...
#             self.audio_projection = simple_gru_v2(a_cfg.input_dim, a_cfg.hidden_dim, 
#                     num_layers=2, output_dim=a_cfg.output_dim, 
#                     dropout_prob=0.1, use_softmax = False, pad_pack=True, device=a_cfg.device)
...
class mmModule(nn.Module): 
    ...
    def init_audio_hidden(self, batch_size=128):
        """
        Tokenizes the texts
        """
        # tokenized = self.text_model._modules[next(iter(self.text_model._modules))].tokenize(texts)
        ...
        h = self.audio_model._modules[next(iter(self.audio_model._modules))].init_hidden(batch_size)
    ...
    def fit(self, 
        ...
        for epoch in trange(epochs, desc="Epoch", disable=not show_progress_bar):
            ...
            h = self.init_audio_hidden(batch_size=128)
            use_gru = False
            ...
            for new_train_index, (sent_features, audio_features, seq_len) in enumerate(train_dataloader):
                ...
                # check this
                # print("[del] scale: ", loss_model.logit_scale)
                # if update_scale:
                #     with torch.no_grad():
                #         #loss_model.logit_scale.data.clamp_(-np.log(100), np.log(100))
                #         loss_model.logit_scale = torch.clamp(loss_model.logit_scale.exp(), min=1.0, max=100.0)
...
class multimodal_loss(nn.Module):
    ...
    def forward(self, sentence_features: Iterable[Dict[str, Tensor]], audio_features: Tensor, seq_len, h):
        ...
        # Y_hat = reps_audio.view(-1, 128)
        # nb_tokens = np.sum(seq_len)

        test_loss = False
        if test_loss:

            target = torch.full((self.batch_size, 768), 0.1,  device=self._target_device)
            reps_sentences = target  / target.norm(dim=1, keepdim=True)
            # print("Target: ", reps_sentences)
            # audio_logits =  (reps_audio @ sent_target.t()) * self.fixed_scale
            
            # target = torch.arange(self.batch_size, dtype=torch.long, device=self._target_device)
        ...
        # ['simcse_loss', 'clip_loss', 'clip_loss_simple'] del
            ...
            if self.scale_type == 'fixed':
                ...
                audio_logits =  (reps_audio @ reps_sentences.t()) * cur_logit_scale
            ...
            if self.scale_type == 'fixed':
                ...
                cur_logit_scale = torch.clamp(self.logit_scale.exp(), min=1.0, max=100.0)
                scores = self.similarity_fct(reps_sentences, reps_audio) * cur_logit_scale
...
def main(args):
    ...
    # audio_model = simple_gru_v2(mmAudioCfg.input_dim, mmAudioCfg.hidden_dim, 
    #                 num_layers=2, output_dim=mmAudioCfg.output_dim, 
    #                 dropout_prob=0.2, use_softmax = False, pad_pack=True, device=mmAudioCfg.device)

    audio_model = LSTMNet(input_dim=mmAudioCfg.input_dim, 
        hidden_dim = mmAudioCfg.hidden_dim,
        output_dim =mmAudioCfg.output_dim,
        n_layers = 2, device=mmAudioCfg.device)
