...
Perform bm25 evaluation of topic task. 
...
def bm25_evaluation(evaluator):
    """
    This is the bm25 oracle for the topic-test set. 
    Ranks all sentences in the evaluator according to a topical query. 
    Inputs:
        evaluator: Evaluator() object of the topic-test set. 
    Outputs: 
        results_bm25: a dict containing the results of the bm25 evaluation. 
    """
    # Preprocess and create a corput to use for bm25. 
    corpus = preprocess(evaluator.all_sents)
    bm25 = BM25Okapi(corpus)
    k=len(evaluator.all_sents)
    ...
    # For each query find the most similar sentences in the episodes. 
    for query_idx in range(len(evaluator.queries)):
        cur_query = evaluator.queries[query_idx]
        print("---This is query: ", cur_query)

        # Get doc scores for query and calculate predicted episodes. 
        ...
        # Get the rank of the highest correctly predicted episode. 
...
def main(args):
    """ Performs bm25 evaluation of the topic-test set. """
    ...
    # Opening configuration file of the run. 
    ...
    # Create dataloader and load the model. 
    ...
    # Performs bm25 evaluation and saves the results. 
